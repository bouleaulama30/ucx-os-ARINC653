...
static inline uint32_t r_stimecmp()
{
	uint32_t x;
...
static inline void w_stimecmp(uint32_t x)
{
	asm volatile("csrw 0x14d, %0" : : "r" (x));
}

/* Machine Environment Configuration Register */
static inline uint32_t r_menvcfg()
{
	uint32_t x;
...
static inline void w_menvcfg(uint32_t x)
{
...
static inline void w_sp(uint32_t x)
{
