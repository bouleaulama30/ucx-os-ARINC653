...
void p1_process1(void)
{
	...
	printf("[P1/Interpartition] queuing_port_id=%d\n", queuing_port_id);
	...
	while (1) {
		...
		if (return_code == NO_ERROR) {
			printf("[P1/Interpartition] RECEIVE_QUEUING_MESSAGE rc=%d len=%d msg='%s'\n",
			       return_code,
			       message_length,
			       message);
		} else {
			printf("[P1/Interpartition] RECEIVE_QUEUING_MESSAGE rc=%d\n", return_code);
		}
...
void p1_process2(void)
{
	...
	while (1) {
		printf("[P1/Interpartition] watcher process\n");
...
void p1_process3(void)
{
	...
	while (1) {
		printf("[P1/Interpartition] periodic process\n");
...
void p2_process1(void)
{
	...
	printf("[P2/Interpartition] queuing_port_id=%d\n", queuing_port_id);
	...
	while (1) {
		...
		if (return_code == NO_ERROR) {
			printf("[P2/Interpartition] SEND_QUEUING_MESSAGE rc=%d seq=%lu len=%d msg='%s'\n",
			       return_code,
			       (unsigned long)seq,
			       message_length,
			       message);
		} else {
			printf("[P2/Interpartition] SEND_QUEUING_MESSAGE rc=%d seq=%lu\n",
			       return_code,
			       (unsigned long)seq);
		}
...
void error_handler_function(void)
{
	...
	printf("[ERROR HANDLER] rc=%d code=%d failed_pid=%d len=%d\n",
	       return_code,
